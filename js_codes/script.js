...
loginBtn.addEventListener("click", (e) => {
  ...
  if (!loginEmail.value.trim()) {
    showError(loginEmail, "Էլփոստի դաշտը պարտադիր է:");
    isValid = false;
  }
  // Validate email domain (only if email is not empty)
  else if (!validateEmailDomain(loginEmail.value)) {
    showError(loginEmail, "Խնդրում ենք օգտագործել համալսարանի էլ. փոստը (օր.` example@polytechnic.am)։");
    isValid = false;
  }
  
  // Validate password not empty
  if (!loginPassword.value.trim()) {
    showError(loginPassword, "Գաղտնաբառի դաշտը պարտադիր է:");
    isValid = false;
  }
...
function validateForgotPasswordEmail() {
  ...
  if (!fpEmail.value.trim()) {
    showError(fpEmail, "Էլփոստի դաշտը պարտադիր է:");
    return false;
  }
  
  if (!validateEmailDomain(fpEmail.value)) {
    showError(fpEmail, "Խնդրում ենք մուտքագրել համալսարանի ճիշտ էլ. փոստ (օր.` example@polytechnic.am)։");
    return false;
  }
...
verifyCodeBtn.addEventListener("click", () => {
  ...
  if (!codePattern.test(code)) {
    showError(fpCode, "Վավերացման կոդը պետք է լինի 6 նիշանոց թվային։");
    return;
  }
...
resetPasswordBtn.addEventListener("click", () => {
  ...
  if (!fpNewPass.value.trim()) {
    showError(fpNewPass, "Նոր գաղտնաբառի դաշտը պարտադիր է:");
    isValid = false;
  }
  
  // Check if confirm password is not empty
  if (!fpConfirmPass.value.trim()) {
    showError(fpConfirmPass, "Հաստատման գաղտնաբառի դաշտը պարտադիր է:");
    isValid = false;
  }
  
  // Check if passwords match
  if (fpNewPass.value.trim() && fpConfirmPass.value.trim() && fpNewPass.value !== fpConfirmPass.value) {
    showError(fpConfirmPass, "Գաղտնաբառերը չեն համընկնում։");
    isValid = false;
  }
  
  // Check password strength (optional)
  if (fpNewPass.value.trim() && fpNewPass.value.length < 6) {
    showError(fpNewPass, "Գաղտնաբառը պետք է պարունակի առնվազն 6 նիշ:");
    isValid = false;
  }
